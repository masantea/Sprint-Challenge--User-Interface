...
Semantic HTML is adding meaning to a webpage, it helps commnunicate what is in the html folder eg headers and paragraps

2. Name two big differences between ```display: block;``` and ```display: inline;```.

display:inline every element is on the same like, with display:block elements start on a new line.

display:inline width and height properties cannot be set, with display:block width and height properties can be set. 
